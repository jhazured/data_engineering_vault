...
#pyspark #testing #fundamentals #core-concepts #unit-testing #test-fixtures #spark-session #test-environment #distributed-testing #data-engineering
...
## 7.1 Core Testing Fundamentals

### Unit Testing Basics for PySpark

**PySpark unit testing** differs from regular Python testing because you're testing distributed computations rather than simple functions. The key challenge is managing the SparkSession lifecycle and ensuring tests run efficiently without spinning up full clusters for every test case.

**SparkSession Management Strategy:** The session-scoped fixture pattern shown below is crucial because creating a new SparkSession for each test is extremely expensive (10-30 seconds per test). Instead, we create one session for the entire test suite and ensure proper cleanup between tests.

**Setting up SparkSession for Testing**
...
def spark():
    ...
    - local[2]: Run locally with 2 threads to simulate parallelism
    - spark.sql.shuffle.partitions=2: Reduce from default 200 to speed up tests
    - session scope: Reuse same SparkSession across all tests in session
    """
...
def sample_data(spark):
    ...
    - Easy to modify schema for different test scenarios
...
- **Performance**: One SparkSession creation instead of hundreds
- **Isolation**: Each test gets fresh data via fixtures
- **Debugging**: Easier to debug with consistent Spark configurations
- **CI/CD Friendly**: Fast test execution suitable for automated pipelines

**Testing Data Transformations**

```python
def test_age_categorization(spark, sample_data):
    """Test age categorization logic
    
    Testing Strategy Explained:
    1. Apply transformation to known test data
    2. Collect results to driver (safe for small test datasets)
    3. Assert against expected outcomes using standard Python assertions
    ...
    # Collect results for assertion (only safe for small test data!)
    result_data = result.select("name", "age_group").collect()
    
    # Assert expected outcomes
    expected = [("Alice", "Young"), ("Bob", "Middle"), ("Charlie", "Middle")]
    actual = [(row.name, row.age_group) for row in result_data]
...
- **collect() Usage**: Only use `.collect()` in tests with small datasets (< 1000 rows). For larger test datasets, use sampling or count-based assertions.
- **Deterministic Results**: The test above is deterministic because we control the input data and the transformation logic has clear boundaries.
- **Edge Case Testing**: Always test boundary conditions (age = 30, age = 40) to ensure your when/otherwise logic is correct.

### Test Environment Configuration

**Environment consistency** is critical for reliable PySpark testing. Production issues often stem from environment differences between development, testing, and production clusters. Containerized testing environments solve this by providing identical runtime conditions across all environments.

**Docker-based Test Environment**

```yaml
# docker-compose.test.yml
# Why Docker for PySpark Testing:
# 1. Consistent Java/Scala/Python versions across team
# 2. Isolated network environment for testing external connections
# 3. Reproducible test failures - same environment every time
# 4. Easy CI/CD integration with container runners
...
services:
  spark-test:
    ...
    environment:
      ...
      # Memory settings for test containers
      - SPARK_DRIVER_MEMORY=1g
      - SPARK_EXECUTOR_MEMORY=1g
    volumes:
      - ./tests:/opt/tests
      - ./src:/opt/src
      - ./test_data:/opt/test_data  # Mount test data directory
    command: pytest /opt/tests/ -v --tb=short
    
  # Optional: Separate worker nodes for distributed testing
  spark-worker:
    image: bitnami/spark:3.4
    environment:
      - SPARK_MODE=worker
...
class TestConfig:
    ...
    - Easy to override for different environments (local, CI, staging)
    ...
    def get_spark_config() -> Dict[str, Any]:
        """Get Spark configuration for testing
        
        Configuration Choices Explained:
        - shuffle.partitions=2: Default 200 is overkill for test data, slows down tests
        - adaptive.enabled=false: Adaptive query execution can make tests non-deterministic
        - KryoSerializer: Faster serialization, catches serialization issues early
        """
        return {
            "spark.app.name": "PySpark-Tests",
            "spark.master": os.getenv("SPARK_MASTER", "local[*]"),
            "spark.sql.shuffle.partitions": "2",
            "spark.sql.adaptive.enabled": "false",  # Disable for consistent tests
    ...
    def get_test_data_path() -> str:
        ...
        - Environment variable takes precedence (for CI/CD flexibility)
        - Fallback to relative path for local development
        - Allows different test data sets for different environments
        """
        return os.getenv("TEST_DATA_PATH", "./test_data/")
    
    @staticmethod
    def should_use_cluster() -> bool:
        """Determine if tests should run on actual cluster vs local
        
        Use Cases:
        - Local development: Always use local[*] for speed
        - CI: Use local[*] for unit tests, small cluster for integration tests
        - Performance testing: Use multi-node cluster
        """
        return os.getenv("USE_CLUSTER", "false").lower() == "true"
```

**Benefits of This Approach:**

- **Consistency**: Same environment every time tests run
- **Debugging**: Easier to reproduce test failures locally
- **CI/CD Integration**: Container-based testing works seamlessly in pipelines
- **Resource Control**: Predictable memory and CPU usage for tests

### Test Data Management Strategies

**Effective test data management** is often the difference between reliable, fast tests and flaky, slow ones. In PySpark testing, you need data that's realistic enough to catch real bugs but small enough to run quickly. The key is creating synthetic data that mirrors production patterns without the complexity.
...
class TestDataGenerator:
    """Generate realistic test data for PySpark testing
    
    Why Generate Instead of Using Production Data:
    1. Speed: Small, controlled datasets run tests in seconds vs minutes
    2. Privacy: No risk of exposing sensitive production data
    3. Edge Cases: Can easily create specific scenarios (nulls, duplicates, skew)
    4. Determinism: Same data every test run for reproducible results
    5. Isolation: Tests don't break when production data changes
    """
    
    def __init__(self, spark: SparkSession):
        self.spark = spark
    
    def generate_sales_data(self, num_rows: int = 1000):
        """Generate synthetic sales data that mirrors real patterns
        
        Design Principles:
        - Include realistic value distributions (not just sequential IDs)
        - Add categorical data for groupBy operations testing
        - Include edge cases like high/low values naturally
        - Use deterministic randomness for reproducible tests
        """
        # Create base DataFrame with random but realistic values
        df = self.spark.range(num_rows).select(
            col("id").cast("string").alias("transaction_id"),
            # Amount: mostly $100-1100, with some outliers
            (rand() * 1000 + 100).cast("decimal(10,2)").alias("amount"),
            # Quantity: normal distribution around 10, range 5-15 mostly
            (randn() * 5 + 10).cast("int").alias("quantity"),
            # Tier: 70% Standard, 30% Premium (realistic business distribution)
            when(rand() > 0.7, "Premium").otherwise("Standard").alias("tier")
        )
        
        # Add realistic date column spanning a year
        # Why not use random dates: Creates realistic time-series patterns
        base_date = datetime(2023, 1, 1)
        dates = [base_date + timedelta(days=random.randint(0, 365)) 
                for _ in range(num_rows)]
        
        date_df = self.spark.createDataFrame(
            [(i, date) for i, date in enumerate(dates)], 
            ["id", "transaction_date"]
        )
        
        return df.join(date_df, "id").drop("id")
    
    def generate_skewed_data(self, num_rows: int = 10000):
        """Generate data with realistic skew patterns
        
        Why Test Data Skew:
        - Production data is rarely evenly distributed
        - Skew causes performance issues and job failures
        - Tests should catch skew-related problems early
        """
        # 80% of data in 20% of partitions (classic 80/20 distribution)
        skew_keys = ["popular_key"] * int(num_rows * 0.8) + \
                   [f"rare_key_{i}" for i in range(int(num_rows * 0.2))]
        
        return self.spark.createDataFrame(
            [(i, skew_keys[i % len(skew_keys)], rand()) 
             for i in range(num_rows)],
            ["id", "partition_key", "value"]
        )
```
...
def anonymize_sensitive_data(df, sensitive_columns):
    """Anonymize sensitive data for testing
    
    When to Use This Pattern:
    - You need to test with production-like data volumes
    - Production data contains PII that can't be in test environments
    - You need realistic data distributions but not actual values
    
    Anonymization Strategy:
    - Hash sensitive fields with salt to prevent rainbow table attacks
    - Preserve data types and null patterns
    - Maintain referential integrity (same input -> same hash)
    """
    anonymized_df = df
    ...
    for col_name in sensitive_columns:
        if col_name in df.columns:
            ...
            # SHA2-256 ensures one-way transformation
            anonymized_df = anonymized_df.withColumn(
                col_name,
                sha2(concat_ws("_", col(col_name), lit("salt_key_2023")), 256)
            )
    
    return anonymized_df

# Example usage in tests
def test_with_anonymized_data(spark):
    """Example of testing with anonymized production data"""
    # Original sensitive data
    sensitive_df = spark.createDataFrame([
        (1, "john.doe@email.com", "123-45-6789"),
        (2, "jane.smith@email.com", "987-65-4321")
    ], ["id", "email", "ssn"])
    
    # Anonymize for testing
    test_df = anonymize_sensitive_data(sensitive_df, ["email", "ssn"])
    
    # Test still works with anonymized data
    result = test_df.groupBy("id").count()
    assert result.count() == 2  # Structure preserved, data protected
```

**Benefits of This Approach:**

- **Performance**: Small datasets mean fast tests
- **Reliability**: Controlled data eliminates environmental test failures
- **Coverage**: Easy to create edge cases that are rare in production
- **Privacy**: No risk of exposing sensitive data in test environments
- **Maintenance**: Tests don't break when production data schemas change

---


## 7.3 Data Quality & Validation Testing
...
#pyspark #testing #data-quality #performance #ci-cd #monitoring #fundamentals #data-engineering #spark #big-data
